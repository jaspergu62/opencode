...
The overlay must remain compatible with the PACT runtime parser. A terminal review uses the exact headings \`### Behavioral Contract Audit\`, \`### Base-Equivalence Proof Audit\`, \`### Status Delta\`, and \`### Complete Decision Evidence\`. In the Behavioral Contract Audit, spell out every \`BO-*\` ID individually with one exact terminal proof status and concrete evidence; compressed ranges such as \`BO-010..BO-017\`, grouped shorthand, or a JSON-only obligations map do not satisfy the gate. The Status Delta heading is followed by one fenced JSON object with \`role: "reviewer_confirmed"\`. Its durable parser fields are \`ac\`, \`tasks\`, \`approved\`, and \`rejected\`; other fields are advisory. AC statuses are exactly \`met\`, \`partial\`, \`not_met\`, \`deferred\`, or \`blocked\`; task statuses are exactly \`complete\`, \`partial\`, \`pending\`, \`deferred\`, or \`blocked\`. Do not redefine these vocabularies. \`PACT_COMPLETE\` is valid only as the final non-empty line.
